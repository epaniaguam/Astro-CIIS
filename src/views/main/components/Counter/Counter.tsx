...
const Counter = ({ eventDate }: PropsCounter) => {
  ...
  return (
    <div className='flex gap-x-0 sm:gap-x-3 justify-center text-5xl mt-8 transition-transform '>
      <CardCounter
        time={time.remainDays}
        description='dias'
      />
      <CardCounter
        time={time.remainHours}
        description='horas'
      />
      <CardCounter
        time={time.remainMinutes}
        description='minutos'
      />
      <CardCounter
        time={time.remainSeconds}
